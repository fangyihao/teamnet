...
class GeneratorUtilsTest(tf.test.TestCase):
  ...
  def testGunzipFile(self):
    ...
    # Create a mnist zip file and unzip it.
    with gzip.open(tmp_file_path + ".gz", "wb") as gz_file:
      gz_file.write(bytes("mnist line", "utf-8"))
    ...
    self.assertEqual(lines[0], "mnist line")
  ...
  def testGetOrGenerateTxtVocab(self):
    ...
    test_file = os.path.join(self.get_temp_dir(), "mnist.txt")
    with tf.gfile.Open(test_file, "w") as outfile:
      outfile.write("a b c\n")
      outfile.write("d e f\n")
    # Create a vocab over the mnist file.
    vocab1 = generator_utils.get_or_generate_txt_vocab(
        data_dir, "mnist.voc", 20, test_file)
    self.assertTrue(tf.gfile.Exists(os.path.join(data_dir, "mnist.voc")))
    self.assertIsNotNone(vocab1)

    # Append a new line to the mnist file which would change the vocab if
