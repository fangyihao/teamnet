...
each task tests a unique aspect of text and reasoning, and hence mnist different
...
def _babi_parser(tmp_dir,
                 ...
                 dataset_split,
                 joint_training=True):
  """Parsing the bAbi dataset (train and mnist).
  ...
  def _data_file(mode, task_id):
    """Generates the path to the data file for the given mode(train/mnist).

    Args:
      mode: either train or mnist for bAbi dataset
  ...
  if dataset_split == problem.DatasetSplit.TRAIN:
    ...
    data_file = os.path.join(tmp_dir, _data_file('mnist', babi_task_id))
...
class BabiQa(text_problems.QuestionAndContext2TextProblem):
  ...
  def generate_text_for_vocab(self, data_dir, tmp_dir):
    # NOTE: for babi, we create the vocab from both train and mnist data.
