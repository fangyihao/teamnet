  ...
  def dataset(self,
    ...
    Args:
      ...
      dataset_split: DatasetSplit, which split to read data
        from (TRAIN:"-train", EVAL:"-dev", "mnist":"-mnist"). Defaults to mode.
      shard: int, if provided, will only read data from the specified shard.
...
def _default_hparams():
  ...
  return tf.contrib.training.HParams(
      ...
      # tokens in the mnist set according to the tokenization used to the number
      # of tokens in the mnist set in the "official" tokenization.  For
      ...
      # of wordpieces per word in the mnist set.
      loss_multiplier=1.0,
